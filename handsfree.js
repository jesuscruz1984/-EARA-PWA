// EARA hands-free wake phrase: Eara
(()=>{
  ...
  function armActive(){
    ...
    clearActiveTimer();
    activeTimer=setTimeout(checkActiveTimeout,ACTIVE_MS+100);
    if(!speaking&&!processing){setState(idleState());setBadge(idleBadge())}
  }
  function checkActiveTimeout(){
    if(!active)return;
    const remain=activeUntil-Date.now();
    if(remain>0){activeTimer=setTimeout(checkActiveTimeout,remain+100);return}
  ...
  function submitCommand(){
    clearTimeout(commandTimer);commandTimer=null;
    if(!commandBuffer||processing||speaking){if(commandBuffer)commandTimer=setTimeout(submitCommand,300);return}
    ...
    Promise.resolve(window.askAI?.(cmd)).catch(()=>{}).finally(()=>{
      processing=false;
      if(active)armActive();
      if(handsFree&&!speaking&&micAvailable())scheduleRestart(120);
    });
  }

  function scheduleRestart(delay=180){
    clearTimeout(restartTimer);
    if(!handsFree||speaking||processing||!recognition||!micAvailable())return;
    const backoff=Math.min(2200,speechErrorCount*300);
    restartTimer=setTimeout(()=>{
      if(!handsFree||speaking||processing||starting||!micAvailable())return;
      try{starting=true;recognition.start()}
      catch(_){starting=false;speechErrorCount=Math.min(speechErrorCount+1,6);scheduleRestart(400)}
  ...
  function setupRecognition(){
    ...
    recognition.onstart=()=>{
      starting=false;speechErrorCount=0;
      if(handsFree&&!speaking&&!processing){setState(idleState());updateSpeakerButton();setBadge(idleBadge())}
    };
    ...
    recognition.onresult=e=>{
      ...
      for(let i=e.resultIndex;i<e.results.length;i++){
        ...
        const heard=pickTranscript(res);
        if(!heard)continue;
        const wake=heard.match(WAKE);
        if(wake){
          armActive();
          if(!res.isFinal){setState('Eara heard — I’m listening…');setBadge('Eara Active');continue}
          const rest=String(wake[1]||'').trim();
          if(rest)appendCommand(rest);
          else{setState('Yes? I’m listening…');setBadge('Eara Active')}
          continue;
        }
        if(!active)continue;
        armActive();
        if(res.isFinal)appendCommand(heard);
        else{setState('Eara active — listening…');setBadge('Eara Active')}
      }
    };
    ...
    recognition.onerror=e=>{
      ...
      if(err==='not-allowed'||err==='service-not-allowed'){
        if(micAvailable()){
          ...
          setState('Reconnecting listener…');setBadge('Listener reconnecting');scheduleRestart(650);
        }else{
          handsFree=false;setState('Microphone permission required.');setBadge('Mic Permission');
        }
        return;
      }
      if(err==='audio-capture'&&!micAvailable()){setState('Microphone unavailable.');return}
      speechErrorCount=Math.min(speechErrorCount+1,6);
      if(!speaking&&!processing)scheduleRestart(err==='no-speech'?100:350);
    };

    recognition.onend=()=>{
      starting=false;
      if(handsFree&&!speaking&&!processing)scheduleRestart(active?80:180);
    };
  }
  ...
  function localSpeak(text,onDone){
    ...
    try{
      ...
      u.lang='en-US';u.volume=1;u.rate=1;
      const voices=speechSynthesis.getVoices();
      const v=voices.find(x=>/^en-US$/i.test(x.lang)&&/Samantha|Ava|Aaron|Alex|Siri/i.test(x.name))||voices.find(x=>/^en/i.test(x.lang));
      if(v)u.voice=v;
      let finished=false;
      const finish=()=>{if(finished)return;finished=true;if(typeof onDone==='function')onDone()};
  ...
  async function primeAudio(){
    ...
    try{
      ...
      p.volume=0.01;
      await p.play();
      p.pause();p.currentTime=0;p.volume=1;
      audioPrimed=true;
      return true;
    }catch(_){return false}
  }
  ...
  function revokeObjectUrl(){
    if(currentObjectUrl){try{URL.revokeObjectURL(currentObjectUrl)}catch(_){}currentObjectUrl=''}
  }

  function audioBlobFromBase64(b64,mime){
  ...
  async function playCloudAudio(audio,mime,onDone,msg){
    const p=ensurePlayer();
    try{p.pause()}catch(_){}
    revokeObjectUrl();
    currentObjectUrl=URL.createObjectURL(audioBlobFromBase64(audio,mime));
    p.src=currentObjectUrl;
    p.volume=1;
    let finished=false;
    const finish=()=>{if(finished)return;finished=true;revokeObjectUrl();onDone()};
    p.onended=finish;
    p.onerror=()=>{revokeObjectUrl();if(!localSpeak(msg,finish))finish()};
    try{
      p.load();
      await p.play();
      audioPrimed=true;
      ...
      revokeObjectUrl();
      if(localSpeak(msg,finish))return true;
      finish();
      return false;
    }
  }
  ...
  async function cloudSpeak(text){
    ...
    if(!msg||!speakerEnabled){if(active)armActive();if(handsFree&&micAvailable())scheduleRestart(100);return}

    speaking=true;
    clearTimeout(restartTimer);
    try{recognition?.abort()}catch(_){}
    setState('EARA speaking…');setBadge('Speaking');
    ...
    const done=()=>{
      ...
      if(handsFree&&micAvailable()){
        setState(idleState());setBadge(idleBadge());scheduleRestart(120);
      }
    };
    ...
    const finish=()=>{clearTimeout(watchdog);done()};

    try{
      const r=await fetch('https://eara-pwa.jesuscruz1984.workers.dev/tts',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({text:msg,speaker:window.getEaraVoice?.()||'asteria'}),cache:'no-store'
      });
      const raw=await r.text();
      if(!r.ok)throw new Error(raw);
      const j=JSON.parse(raw);
      if(!j.audio)throw new Error('No audio returned');
      const played=await playCloudAudio(j.audio,j.mime||'audio/mpeg',finish,msg);
      if(!played)finish();
    }catch(_){
      clearTimeout(watchdog);
      if(!localSpeak(msg,done)){
        speaking=false;
        setState(audioPrimed?'Voice playback failed — tap Speaker OFF then ON once':'Tap anywhere once to unlock iPhone audio, then Eara will speak automatically.');
        setBadge(audioPrimed?'Voice Error':'Audio Tap Needed');
        if(handsFree&&micAvailable())scheduleRestart(160);
      }
    }
  }
  ...
  function enable(){
    setupRecognition();
    if(!SR){setState('Hands-free recognition is not supported in this browser.');return}
    handsFree=true;speechErrorCount=0;updateSpeakerButton();scheduleRestart(60);primeAudio();
  }
  function disable(){
    handsFree=false;clearTimeout(restartTimer);clearTimeout(commandTimer);clearActiveTimer();
    starting=false;active=false;commandBuffer='';
    try{recognition?.abort()}catch(_){}
  }
  function recoverListening(){
    if(!micAvailable())return;
    setupRecognition();handsFree=true;speechErrorCount=0;
    if(!speaking&&!processing)scheduleRestart(50);
  }
  ...
  if(talk)talk.addEventListener('click',async e=>{
    ...
    speakerEnabled=!speakerEnabled;updateSpeakerButton();
    if(!speakerEnabled){
      try{player?.pause()}catch(_){}revokeObjectUrl();
      try{speechSynthesis.cancel()}catch(_){}
      setState(active?'Eara active — speaker off':'Listening for “Eara”');setBadge('Speaker Off');recoverListening();
    }else{
      await primeAudio();recoverListening();setState(idleState());setBadge(idleBadge());
    }
  },true);
