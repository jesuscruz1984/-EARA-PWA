...
(()=>{
  ...
  let recognition=null,handsFree=false,speaking=false,starting=false,restartTimer=null,speechTimer=null,armedUntil=0;
  ...
  function unlockSpeech(){try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(' ');u.volume=0;speechSynthesis.speak(u)}catch(_){}}
  window.say=function(text){const msg=String(text||'').trim();if(!msg)return;speaking=true;clearTimeout(restartTimer);clearTimeout(speechTimer);if(recognition){try{recognition.abort()}catch(_){}}try{speechSynthesis.cancel();speechSynthesis.resume()}catch(_){}const u=new SpeechSynthesisUtterance(msg);u.volume=1;u.rate=1;u.pitch=1;u.lang='en-US';try{const vs=speechSynthesis.getVoices();const v=vs.find(x=>/^en-US$/i.test(x.lang)&&/Samantha|Siri|Ava|Aaron|Alex/i.test(x.name))||vs.find(x=>/^en/i.test(x.lang));if(v)u.voice=v}catch(_){}let doneFlag=false;const done=()=>{if(doneFlag)return;doneFlag=true;clearTimeout(speechTimer);speaking=false;if(handsFree&&micAvailable()){setState('Listening — say “Hey Robot”');scheduleRestart(650)}};u.onstart=()=>{setState('EARA speaking…');try{badge('Speaking')}catch(_){}};u.onend=done;u.onerror=done;setTimeout(()=>{try{speechSynthesis.resume();speechSynthesis.speak(u)}catch(_){done()}speechTimer=setTimeout(done,Math.max(7000,Math.min(40000,msg.split(/\s+/).length*700+4000)))},400)};
  function enable(){setup();if(!SR){setState('Hands-free recognition is not supported in this browser.');return}unlockSpeech();handsFree=true;setTalk('Hey Robot Ready');scheduleRestart(250)}
  ...
  const talk=document.querySelector('#talk');if(talk)talk.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();unlockSpeech();if(!micAvailable())return;if(handsFree){disable();setTalk('Resume Hey Robot');setState('Hands-free paused')}else enable()},true);
