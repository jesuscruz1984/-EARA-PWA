  ...
  async fetch(request, env) {
    ...
    try{
      ...
      if(url.pathname==="/tts"&&request.method==="POST"){
        ...
        const raw=await env.AI.run("@cf/deepgram/aura-1",{text:String(text).slice(0,3500),speaker:voice,encoding:"mp3"},{returnRawResponse:true});
        let audio;if(raw instanceof Response)audio=await raw.arrayBuffer();else if(raw instanceof ArrayBuffer)audio=raw;else if(ArrayBuffer.isView(raw))audio=raw.buffer;else if(raw?.audio&&typeof raw.audio==="string")return j({audio:raw.audio,mime:"audio/mpeg",speaker:voice},200,cors);
        if(!audio)return j({error:"Aura returned no playable audio."},500,cors);
        return j({audio:bytesToBase64(new Uint8Array(audio)),mime:"audio/mpeg",speaker:voice},200,cors)
      }
      if(url.pathname==="/chat"&&request.method==="POST"){
        ...
        if(wantsWeb){
          ...
          try{
            ...
            return j({text:extract(fallback)||"Live web search is not enabled yet.",visionUsed:hasImage,webUsed:false,webError:String(webErr?.message||webErr),model:"@cf/openai/gpt-oss-120b"},200,cors)
          }
        }
        ...
        return j({text:answer||"I couldn't generate a response.",visionUsed:hasImage,webUsed:false,model:"@cf/openai/gpt-oss-120b"},200,cors)
      }
