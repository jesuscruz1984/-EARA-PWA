  ...
  async fetch(request, env) {
    ...
    try{
      if(url.pathname==='/health')return j({
        ok:true,
        service:'EARA fast resilient backend',
      ...
      if(url.pathname==='/tts'&&request.method==='POST'){
        ...
        try{
          const raw=await env.AI.run('@cf/deepgram/aura-1',{text:spoken.slice(0,165),speaker:voice,encoding:'mp3'},{returnRawResponse:true});
        ...
        if(wantsWeb){
          if(env.TAVILY_API_KEY){
            try{
              ...
              if(answer)return chatReply(answer,{visionUsed:!!scene,webUsed:true,webProvider:'Tavily',model:`Tavily direct${visionModel?' + '+visionModel:''}`},cors,'I found it online. The links and details are on screen.');
            }catch(_){}
          }
          try{
      ...
      return j({ok:true,service:'EARA fast resilient backend',reasoning:'Fast Gemma routing + GPT-OSS 120B complex reasoning',vision:'Gemma 4 with Llama fallback',voice:'Deepgram Aura raw audio with iPhone fallback',web:'Tavily direct'},200,cors);
...
function formatTavilyAnswer(data,scene){
  const rows=(data?.results||[]).filter(x=>x?.url).slice(0,4);if(!rows.length)return '';
...
function makeSpeech(input,web=false){
  ...
  let first=(clean.match(/[^.!?]+[.!?]+|[^.!?]+$/)||[clean])[0].trim().replace(/[:;,\-\s]+$/,'.');if(!first||first.length<3)first='I found the information.';
  if(first.length>115){first=first.slice(0,115);const cut=first.lastIndexOf(' ');if(cut>80)first=first.slice(0,cut);first=first.replace(/[,;:\-\s]+$/,'.')}
  if((web||hadLink)&&price&&!first.includes(price))first+=` One price I found is ${price}.`;if(web||hadLink)first+=' The links are on screen.';return first.replace(/\s+/g,' ').trim().slice(0,160);
}
