  ...
  async fetch(request, env) {
    ...
    try{
      ...
      if(url.pathname==="/tts"&&request.method==="POST"){
        ...
        const audio=await env.AI.run("@cf/myshell-ai/melotts",{prompt:String(text).slice(0,2200),lang:"en"});
        if(audio instanceof Response){const h=new Headers(audio.headers);h.set("Access-Control-Allow-Origin",origin===allowedOrigin?origin:allowedOrigin);h.set("Cache-Control","no-store");return new Response(audio.body,{status:audio.status,headers:h})}
        if(audio instanceof ArrayBuffer)return new Response(audio,{headers:{...cors,"Content-Type":"audio/mpeg","Cache-Control":"no-store"}});
        if(ArrayBuffer.isView(audio))return new Response(audio.buffer,{headers:{...cors,"Content-Type":"audio/mpeg","Cache-Control":"no-store"}});
        if(audio?.audio){const raw=typeof audio.audio==='string'?Uint8Array.from(atob(audio.audio),c=>c.charCodeAt(0)):audio.audio;return new Response(raw,{headers:{...cors,"Content-Type":"audio/mpeg","Cache-Control":"no-store"}})}
        return j({error:"TTS returned an unsupported response."},500,cors)
      }
      if(url.pathname==="/chat"&&request.method==="POST"){
        ...
        const system=`You are EARA, a hands-free AI assistant running inside a live camera-and-microphone app. The user's spoken audio is transcribed before it reaches you, so when they ask whether you can hear them, answer naturally that yes, EARA can hear them through the microphone/transcription pipeline. Never claim you cannot hear audio merely because your input arrives as text. ${style} Answer naturally because responses are spoken aloud. When an image is supplied, inspect it carefully and use what you actually see. IMPORTANT: use the memory below to recall prior questions, answers, people, objects, places, links, and visual observations when relevant. Do not claim you forgot something if it is present in memory. You do not currently have live web browsing in this build, so for current online shopping or live web results say that briefly instead of giving a long generic disclaimer.\n\nEARA MEMORY:\n${memory||"(no stored memory yet)"}`;
        let result;
