...
function memoryContext(query){const q=words(query),arr=memory().filter(m=>!staleCapability.test(String(m.assistant||'')));const scored=arr.map((m,i)=>{let score=i/Math.max(1,arr.length)*2;const hay=words((m.user||'')+' '+(m.assistant||''));q.forEach(w=>{if(hay.has(w))score+=4});return {m,score}}).sort((a,b)=>b.score-a.score).slice(0,16).map(x=>x.m).reverse();return scored.map(m=>`[${new Date(m.ts).toLocaleString()}] User: ${m.user}\nEARA: ${m.assistant}${m.vision?'\n[Vision frame processed]':''}`).join('\n\n')}
...
async function startScreenShare(){
  ...
  if(!screenShareSupported()){
    ...
    $('#state').textContent=msg;badge('Screen Share Unavailable');setReply(msg);lastSpeech='iPhone requires the native EARA app for live screen sharing.';if(window.say)window.say(lastSpeech);return false;
  }
  try{
    const ds=await navigator.mediaDevices.getDisplayMedia({video:true,audio:false});
    const track=ds.getVideoTracks()[0];if(!track)throw new Error('No screen video track returned');
    screenStream=ds;screenSharing=true;video.srcObject=screenStream;await video.play();
    $('#screenBtn').textContent='Stop Screen Share';$('#cam').textContent='SCREEN VISION';$('#state').textContent='Screen sharing — say “Hey Robot”';badge('Screen Live');
    track.addEventListener('ended',()=>stopScreenShare(),{once:true});return true;
  }catch(e){
    if(e?.name==='NotAllowedError'){$('#state').textContent='Screen sharing was canceled or blocked.';badge('Live');return false}
    $('#state').textContent='Screen share error: '+(e?.message||e);badge('Screen Error');return false;
  }
}
async function stopScreenShare(){
  if(screenStream){screenStream.getTracks().forEach(t=>{try{t.stop()}catch(_){}});screenStream=null}
  screenSharing=false;$('#screenBtn').textContent='Share Screen';
  if(stream){video.srcObject=stream;try{await video.play()}catch(_){}}
  $('#cam').textContent=cameraEnabled?'WIDE VISION':'CAMERA PAUSED';$('#state').textContent=micEnabled?'Listening — say “Hey Robot”':'Microphone disabled';badge('Live');
}
function snap(){if(screenSharing){const c=$('#canvas');c.width=video.videoWidth||1280;c.height=video.videoHeight||720;c.getContext('2d').drawImage(video,0,0,c.width,c.height);return c.toDataURL('image/jpeg',.72)}if(!stream||!cameraEnabled)return null;const c=$('#canvas');c.width=video.videoWidth||960;c.height=video.videoHeight||540;c.getContext('2d').drawImage(video,0,0,c.width,c.height);return c.toDataURL('image/jpeg',.72)}
function fail(e){setReply('Connection error: '+(e.message||e));$('#state').textContent='Error';badge('Error');$('#ai').textContent='Error'}
async function askAI(text){const q=String(text||'').trim();if(!q)return;try{$('#state').textContent='Thinking…';badge('Thinking');const image=snap();const persona=localStorage.getItem(PERSONAKEY)||'helpful';const mem=memoryContext(q);const requestText=screenSharing?`SCREEN SHARE ACTIVE. The attached image is the user's current shared screen, not the camera. Inspect the screen and answer the user's request: ${q}`:q;const r=await fetch(backend()+'/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:requestText,image,memory:mem,personality:persona,visionActive:!!image})});const raw=await r.text();if(!r.ok)throw new Error('HTTP '+r.status+': '+raw);const j=JSON.parse(raw),answer=j.text||'No reply';lastSpeech=String(j.speech||answer);setReply(answer);addMemory({user:q,assistant:answer,vision:!!image,persona,source:screenSharing?'screen':'camera'});$('#transcript').textContent='You: '+q;$('#ai').textContent=j.webUsed?'WEB + AI':(screenSharing?'SCREEN + AI':(image?'VISION + AI':'AI ready'));badge(screenSharing?'Screen Live':'Live');if(window.say)window.say(lastSpeech);return answer}catch(e){fail(e);throw e}}
...
const controls=$('#mediaControls');if(controls){const wrap=document.createElement('div');wrap.style.marginTop='10px';wrap.innerHTML='<div class="small" style="margin-bottom:6px">Agent personality</div><select id="personaLiveSelect" style="width:100%"><option value="helpful">Helpful Assistant</option><option value="concise">Fast & Concise</option><option value="expert">Expert Analyst</option><option value="companion">Friendly Companion</option><option value="fieldtech">Field Technician</option><option value="observer">Curious Observer</option></select><div class="small" style="margin:10px 0 6px">Voice</div><select id="voiceLiveSelect" style="width:100%"><option value="asteria">Asteria</option><option value="luna">Luna</option><option value="athena">Athena</option><option value="stella">Stella</option><option value="angus">Angus</option><option value="orion">Orion</option><option value="perseus">Perseus</option><option value="zeus">Zeus</option><option value="helios">Helios</option><option value="hera">Hera</option><option value="arcas">Arcas</option><option value="orpheus">Orpheus</option></select><div class="small" style="margin-top:8px">AI: GPT-OSS 120B reasoning + live vision. Web search uses Tavily first.</div>';controls.insertAdjacentElement('afterend',wrap);$('#personaLiveSelect').onchange=e=>setPersona(e.target.value);$('#voiceLiveSelect').onchange=e=>setVoice(e.target.value)}
...
const initialPersona=localStorage.getItem(PERSONAKEY)||'helpful';setPersona(initialPersona);setVoice(localStorage.getItem(VOICEKEY)||'asteria');renderMemory();
